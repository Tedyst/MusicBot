...
#######################################################################################################################
    ...
    def _gen_embed(self):
        ...
        e = discord.Embed()
        e.colour = 7506394
        e.set_footer(text='Tedyst/MusicBot ({})'.format(BOTVERSION),
                     icon_url='https://i.imgur.com/gFHBoZA.png')
        e.set_author(name=self.user.name, url='https://github.com/Tedyst/MusicBot',
    ...
    async def cmd_follow(self, player, message, author, user_mentions):
        ...
        for i in target.activities:
            if str(i) == "Spotify":
                ...
                self.following[message.guild.id] = [
                    target.id, None, message.channel]
                player.skip()
                await self.safe_delete_message(message, quiet=True)
        ...
        try:
            for guild_id in self.following:
                if self.following[guild_id][0] == after.id:
                    ...
                    log.info(
                        "Added song {} in guild {} because of /follow".format(song_url, guild_id))
                    player.playlist.clear()
